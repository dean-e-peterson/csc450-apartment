...
export default function User({ user, setUsers, units, locations, authUser }) {
  ...
  const [location, setLocation] = useState(
    user.unit ? user.unit.location._id : ""
  );
  ...
  const onCancel = () => {
    ...
    if (user._id.substr(0, 3) === "new") {
      setUsers(prevUsers =>
        prevUsers.filter(prevUser => prevUser._id.substr(0, 3) !== "new")
      );
    }
    setEdit(false);
  };
  ...
  const onSubmit = async e => {
    ...
    try {
      ...
      let response;
      if (user._id.substr(0, 3) === "new") {
        response = await axios.post("api/users/", body, {
          headers: {
        ...
        response = await axios.patch("api/users/" + user._id, body, {
          headers: {
            "x-auth-token": authUser.token,
            "Content-type": "application/json"
          }
            ...
            <Grid container spacing={2}>
              ...
              <Grid item>
                ...
                >
                  <MenuItem value=''>&nbsp;</MenuItem>
                  {units
                    .filter(unit => unit.location._id === location)
                    .map(unit => (
